/**
 * Профиль оружия юнита (weapon datasheet) из правил Warhammer 40,000 10-й редакции.
 * Хранит все характеристики оружия: дальность, тип (Melee/Ranged), атаки, BS/WS, сила,
 * AP, урон (через DamageValue — фиксированный или кубик), ключевые слова оружия и фракции.
 *
 * Реализует правила стрельбы и ближнего боя: дальность, тип атаки, попадание, ранение,
 * пробитие брони, урон. Поддерживает переменный урон (D3, D6+2 и т.д.) через интерфейс
 * DamageValue и ключевые слова оружия (Assault, Devastating Wounds, Lethal Hits и т.д.).
 *
 * Используется в Datasheet как список оружия юнита. Конкретные оружия создаются через
 * фабричные методы (EaglesScream.create(), WatchersAxe.create() и т.д.).
 */
